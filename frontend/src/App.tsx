  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("http://localhost:3000/api/users/me/bio", {
        ...
        if (!res.ok) {
          throw new Error(`Failed to fetch bio: ${res.status}`);
        }
        return res.json();
      })
    ])
      .then(([userData, bioData]) => {
        ...
        if (!completion.isComplete) {
          ...
          navigate("/profile", {
            state: {
              warning: `Please complete your profile (Missing: ${missing}) to access this feature.`
            }
          });
        } else {
          setIsAllowed(true);
...
function App() {
  ...
  useEffect(() => {
    ...
    // Connect to Socket.IO for real-time notifications (no polling - pure real-time)
    ...
    // Cleanup function - guaranteed to run regardless of fetchMe success/failure
    return () => {
      if (socket) {
        socket.disconnect();
      }
      ...
      <div className="app-shell">
        ...
        <Routes>
          ...
          <Route path="/admin/reports" element={
            <AdminRoute element={<AdminReports />} />
          } />
