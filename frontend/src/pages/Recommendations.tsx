...
import { fetchMe, fetchRecommendations, sendConnectionRequest, dismissUser, fetchUserProfile } from "../utils/api";
    ...
    fetchMe(token).then(me => {
      ...
      fetchRecommendations(token)
        .then((data: any[]) => {
          ...
          const promises = data.map(async (userId: number) => {
            const profile = await fetchUserProfile(token, String(userId));
            return {
              userId: userId,
              username: profile.username,
              profilePic: profile.profilePic,
              bio: profile.aboutMe?.bio,
              highlights: profile.aboutMe?.interests || [],
              score: 0,
            };
          });

          Promise.all(promises).then(normalized => {
            setRecommendations(normalized);
          });
        })
        .catch((err) => {
          console.error(err);
          setError("Unable to reach the recommendations service.");
        })
