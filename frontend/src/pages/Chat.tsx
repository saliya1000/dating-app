...
import { fetchMe, fetchConnections, fetchConnection, fetchChatHistory, markMessagesRead } from "../utils/api";
...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    fetchMe(token).then((me) => {
      if (me.error) return;
      setUser(me);
      fetchConnections(token).then(async (connectionIds: number[]) => {
        if (Array.isArray(connectionIds)) {
          ...
          // Only show accepted connections in chat (chat is only possible between connected profiles)
          const acceptedConns = connections.filter((c: any) => c.status === "accepted");
          const cs: ChatUser[] = acceptedConns.map((c: any) => {
                ...
                {chatMessages.map((m) => (
                  <div key={m.id} className={`chat-message ${user.id === m.senderId ? "sent" : "received"}`}>
                    <div className="chat-bubble">
                      ...
                      <span className="chat-timestamp" title={new Date(m.createdAt).toLocaleString()}>
