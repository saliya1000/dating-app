...
import { fetchMe, fetchUserBio, updateUserProfile, updateUserBio, fetchUserProfile, deleteConnection, submitReport } from "../utils/api";
...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    // Check for redirect warning/error from App.tsx
    if (location.state?.warning) {
      setWarningMessage(location.state.warning);
      setShowWarningModal(true);
      // Clear history state to prevent showing it again on refresh
    ...
    if (isMe) {
      ...
      // Fetch profile details from /users/:id/profile endpoint (full details)
      // Note: /users/:id returns only name and profileLink
      Promise.all([
        fetchUserProfile(token, id),
        fetchUserBio(token, id)
      ])
        .then(([user, userBio]) => {
          if (user?.error || user?.message) {
            setError(user.error || user.message);
            setLoading(false);
            return;
          }
          if (!user) {
  ...
  const handleBioSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (pendingLocation) {
        await updateUserProfile(token, pendingLocation);
        setUser(prev => prev ? { ...prev, ...pendingLocation } : null);
  ...
  const handleCityChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (city) {
      if (city.name === "Use GPS") {
        ...
        // Store location change locally instead of saving immediately
        setPendingLocation({ latitude: city.lat, longitude: city.lon });
        // Update user state for UI feedback (optional, but good for immediate feedback)
        setUser(prev => prev ? { ...prev, latitude: city.lat, longitude: city.lon } : null);
        setMessage("Location selected. Click Save to apply.");
      }
    }
  };
  ...
  return (
    <main className="page-surface">
      <section className="card">
        ...
        {isMe && (
          <form className="split-layout mt-lg" onSubmit={handleBasicSubmit}>
      ...
      {showWarningModal && (
        <div className="modal-overlay">
          <div className="card modal-content">
            ...
            <h2 className="section-heading" style={{ color: "var(--error-color)" }}>Action Required</h2>
