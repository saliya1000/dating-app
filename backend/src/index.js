...
io.on("connection", (socket) => {
  ...
  socket.on("chat message", async (msg) => {
    ...
    // Verify connection exists and is accepted, and sender is part of it
    ...
    if (!connection) {
      console.log(`Chat message rejected: User ${senderId} not connected to connection ${connectionId}`);
      return; // Silently reject - user is not connected
    }

    // Verify recipient is the other party in the connection
    const otherUserId = connection.requesterId === senderId ? connection.recipientId : connection.requesterId;
    if (otherUserId !== recipientId) {
      console.log(`Chat message rejected: Recipient ${recipientId} doesn't match connection ${connectionId}`);
      return; // Silently reject - recipient doesn't match connection
    }
